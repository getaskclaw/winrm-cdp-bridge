...
block, then run from the Hermes/Linux side. It writes a PowerShell UI Automation
collector into the active Windows desktop session via WinRM without killing Chrome
or copying profiles.
"""
...
def build_powershell(cfg: dict) -> str:
    ...
    # This template is deliberately conservative. Improve extraction logic per site changes.
    return rf'''
...
# Basic Markdown; caller may post-process timestamps/links with CDP/API enrichment.
$md=@()
